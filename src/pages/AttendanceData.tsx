...
export default function AttendanceData(){
    ...
    const fetchParticipants = async () => {
        try {
            ...
            if (res.ok) {
                const data = await res.json();
                setParticipants(data.filter((u: any) => u.role === "PARTICIPANT"));
            }
        } catch (err) {
    ...
    const fetchAttendance = async (sessionId: number) => {
        ...
        try {
            const token = localStorage.getItem("token");
            const res = await fetch(`/api/events/1/sessions/${sessionId}/attendance`, {
                headers: { Authorization: `Bearer ${token}` },
            });
            if (res.ok) {
                const data = await res.json();
        ...
        <div className="page-wrapper">
            ...
            <main className="container">
                ...
                {loading ? (
                    ...
                    <div className="sessions-container" style={{ display: 'flex', flexDirection: 'column', gap: '15px', maxWidth: '900px', margin: '0 auto' }}>
